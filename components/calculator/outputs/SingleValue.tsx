...
export default function SingleValue({ field, data }: OutputComponentProps) {
  ...
  if (field.highlight) {
    return (
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-brand-50 to-brand-100/60 border border-brand-200 p-5 sm:p-6 text-center">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_rgba(26,111,160,0.06),_transparent_60%)]" />
        <div className="relative">
          <span className="text-xs font-semibold uppercase tracking-widest text-brand-600/80">
            {field.label}
          </span>
          ...
          {field.description && (
            <p className="mt-2.5 text-sm leading-relaxed text-brand-600/70 max-w-md mx-auto">
              {field.description}
            </p>
          )}
        </div>
  ...
  return (
    <div className="flex flex-col items-center rounded-lg bg-gray-50 p-4 text-center">
      <span className="text-xs font-medium uppercase tracking-wider text-gray-500 mb-1">
        {field.label}
      </span>
